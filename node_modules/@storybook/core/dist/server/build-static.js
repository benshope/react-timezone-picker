...
async function compileManager(managerConfig, managerStartTime) {
  ...
  return new Promise((resolve, reject) => {
    (0, _webpack.default)(managerConfig).run((error, stats) => {
      ...
      stats.toJson().warnings.forEach(e => _nodeLogger.logger.warn(e));
      resolve(stats);
    });
  });
}
...
async function buildPreview(configType, outputDir, packageJson, options) {
  ...
  const previewConfig = await (0, _config.default)(Object.assign({
    configType,
    outputDir,
    packageJson,
...
function prepareFilesStructure(outputDir, defaultFavIcon) {
  ...
  _shelljs.default.mkdir('-p', _path.default.join(outputDir, 'sb_dll')); // clear the static dir


  _shelljs.default.rm('-rf', _path.default.join(outputDir, 'static'));
...
function buildStatic(_ref) {
  let {
    packageJson
      ...
      loadOptions = _objectWithoutPropertiesLoose(_ref, ["packageJson"]);

  const cliOptions = (0, _cli.getProdCli)(packageJson);
  return buildStaticStandalone(Object.assign({}, cliOptions, loadOptions, {
    packageJson,
